...
#   curl -fsSL https://terminal.army/install.sh | VERSION=v0.1.0 sh
#
# Or to a custom install directory:
#   curl -fsSL https://terminal.army/install.sh | INSTALL_DIR=$HOME/bin sh
#
...
download() {
    ...
    binary_path=$(find "$tmp" -type f -name "$BINARY_NAME" | head -1)
