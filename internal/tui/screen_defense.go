...
// defenseScreen mirrors shipyardScreen but for defensive structures.
type defenseScreen struct {
	root     *rootModel
	planetID int64
	planet   *svc.Planet
...
func newDefenseScreen(root *rootModel, planetID int64) *defenseScreen {
	in := textinput.New()
	in.Placeholder = "count"
	in.Width = 8
	in.CharLimit = 8
	return &defenseScreen{root: root, planetID: planetID, loading: true, countInp: in}
}

func (s *defenseScreen) Init() tea.Cmd {
	return tea.Batch(
...
func (s *defenseScreen) Update(msg tea.Msg) (Screen, tea.Cmd) {
	switch m := msg.(type) {
	...
	case tea.KeyMsg:
		if s.editing {
			...
			case "enter":
				...
				key := DefenseCatalog[s.cursor].Key
				return s, cmdQueueDefense(s.root.client, s.planetID, key, v)
			}
			var cmd tea.Cmd
		...
		case "down", "j":
			if s.cursor < len(DefenseCatalog)-1 {
				s.cursor++
			}
		case "enter", "u":
			s.editing = true
...
func (s *defenseScreen) View() string {
	st := s.root.styles
	if s.loading || s.planet == nil {
		return st.Muted.Render("loading planet...")
	}
	rows := []string{st.Header.Render("Defense - " + s.planet.Name)}
	for i, item := range DefenseCatalog {
		have := s.planet.Defense[item.Key]
	...
	if s.editing {
		rows = append(rows, "", st.Muted.Render("how many "+DefenseCatalog[s.cursor].Label+"?"), s.countInp.View())
	}
	left := st.Panel.Render(lipgloss.JoinVertical(lipgloss.Left, rows...))

	queueRows := []string{st.Header.Render("Defense queue")}
	if len(s.queues) == 0 {
		queueRows = append(queueRows, st.Muted.Render("(empty)"))
	}
	for _, q := range s.queues {
		if q.QueueType != "defense" {
			continue
		}
...
func (s *defenseScreen) Title() string { return "defense" }

func (s *defenseScreen) Help() []HelpEntry {
	if s.editing {
		return []HelpEntry{
